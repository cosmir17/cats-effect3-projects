...
object VideoEnquirer {
  def make[F[_]: Logger: Sync: MonadCancelThrow: Console](clients: HttpClients[F]): VideoEnquirer[F] =
    new VideoEnquirer[F](clients) {}
}

class VideoEnquirer[F[_]: Sync: Logger: MonadCancelThrow: Console] private (clients: HttpClients[F]) {
  ...
  def saveAsFile(videoResource: Resource[F, BufferedInputStream]): F[Unit] =
    for {
      ...
      fileName <- Console.make.readLine
  ...
  def entityBodyToResource(b: ByteVector): Resource[F, BufferedInputStream] =
    Resource.make {
      Sync[F].delay(new BufferedInputStream(new ByteArrayInputStream(b.toArray))) // build
    } { inStream =>
  ...
  private def validCase(): F[Unit] =
    for {
      _ <- Sync[F].delay(println(s"the video is valid"))
    } yield ()
